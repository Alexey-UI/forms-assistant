...
- [x] Prisma: модель `GroupMessage` (groupId, authorId, text, editedAt?, createdAt), поля `canWrite`/`lastReadAt` на `GroupMembership`
- [x] Миграция

### Эпик 15 — Backend: REST для сообщений

- [x] `GET /groups/:id/messages` (курсорная пагинация), `POST /groups/:id/messages`
- [x] `PATCH /groups/:id/messages/:messageId` (автор), `DELETE /groups/:id/messages/:messageId` (автор или админ)
- [x] `POST /groups/:id/read`, `GET /groups/unread-summary`
- [x] Проверка `canWrite` при отправке, проверка членства в группе на все операции

### Эпик 16 — Backend: управление участниками (админ)

- [x] `PATCH /groups/:id/members/:userId/write-access` (только админ)
- [x] Фронтенд для уже существующих add/remove/role эндпоинтов (сами эндпоинты готовы с Эпика 4)

### Эпик 17 — Backend: realtime-слой (Socket.IO)

- [x] `http.Server` вместо `app.listen`, Socket.IO поверх него, JWT-аутентификация handshake
- [x] Auto-join в комнаты `group:<id>` для своих групп + личная комната `user:<id>`
- [x] Рассылка событий из сервисов после мутаций (новое/изменённое/удалённое сообщение, mute, участники, непрочитанные)

### Эпик 18 — Frontend: клиент чата и стор

- [x] `socket.io-client`, подключение на уровне App (аналогично `initAuth`), обработка reconnect/logout
- [x] Zustand-стор: сообщения по группам, статус соединения, непрочитанные

### Эпик 19 — Frontend: страница группы `/groups/:groupId`

- [x] Кликабельное название группы в профиле → переход на страницу
- [x] Лента сообщений с подгрузкой истории, инпут (disabled + пояснение при mute), редактирование/удаление своих сообщений
- [x] Список участников с ролями и mute-бейджем (видно всем)

### Эпик 20 — Frontend: админ-панель группы

- [x] Пригласить участника (поиск по друзьям, аналогично инвайту в опрос), удалить участника
- [x] Сменить роль ADMIN/MEMBER, замьютить/размьютить — удаление чужих сообщений

### Эпик 21 — Frontend: бейдж непрочитанных

- [x] В списке групп в профиле, обновление в реальном времени через личную комнату сокета

### Эпик 22 — Тесты чата

- [x] Интеграционный тест прав доступа: не-участник не может читать/писать, замьюченный не может писать, удалять чужие сообщения может только автор/админ
