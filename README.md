...
- **Дублирование опроса** — кнопка «Клонировать» создаёт черновик-копию (заголовок, вопросы, варианты) для быстрого повторного использования, не трогая оригинал.
- **Дедлайн** — необязательная дата и время в конструкторе; фоновый шедулер раз в минуту закрывает опубликованные опросы с истёкшим дедлайном.
- **Квиз-режим** — у вопросов с выбором можно отметить правильные варианты; при прохождении считается балл, респондент видит счёт сразу после отправки, автор — средний балл в результатах и в CSV-экспорте.
- **Live-режим результатов** — автор запускает трансляцию, и страница результатов обновляется в реальном времени при каждом новом ответе (тот же Socket.IO-канал, что у уведомлений и чата).
