...
## Возможности для автора опроса

- **Экспорт в CSV** — построчная выгрузка ответов (открывается в Excel/Google Sheets). Респондент указывается только для `NAMED`-опросов — для `ANONYMOUS`/`PUBLIC_LIST` колонка отсутствует, так же как и в остальном приложении.
- **QR-код** — генерируется на клиенте прямо рядом со ссылкой на прохождение, без обращения к бэкенду.
- **Графики в результатах** — bar chart по каждому вопросу с выбором вариантов, поверх уже имеющихся счётчиков/процентов.
- **Напоминание неответившим** — считает разницу между приглашённой аудиторией (прямые приглашения + участники групп, с которыми расшарен опрос) и уже прошедшими (`Participation`), и рассылает уведомление каждому. Недоступно для `ANONYMOUS` — по дизайну анонимности узнать, кто не ответил, невозможно.
- **Уведомления в приложении** — колокольчик в шапке с непрочитанными: заявка в друзья, приглашение на опрос, добавление в группу, новый ответ на свой опрос. Доставляются в реальном времени через тот же Socket.IO-канал, что и групповой чат.
